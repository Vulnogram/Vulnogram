...
d[a]=e.concat(b[a]).reduce(function(a,b){return a.indexOf(b)<0&&a.push(b),a},[]):d[a]=e}),f(b,function(b,c){"undefined"==typeof a[b]&&(d[b]=c)}),d},setCopyClipboardContents:function(a){this.copyClipboard=a},getCopyClipboardContents:function(){return this.copyClipboard}},h.defaults={themes:{},templates:{},iconlibs:{},editors:{},languages:{},resolvers:[],custom_validators:[]},h.Validator=c.extend({init:function(a,b,c){this.jsoneditor=a,this.schema=b||this.jsoneditor.schema,this.options=c||{},this.translate=this.jsoneditor.translate||h.defaults.translate},validate:function(a){return this._validateSchema(this.schema,a)},_validateSchema:function(a,b,c){var d,g,i,j=this,k=[],l=JSON.stringify(b);/*
     * Type Agnostic Validation
     */
// Version 3 `required` and `required_by_default`
if(c=c||"root",
// Work on a copy of the schema
a=e({},this.jsoneditor.expandRefs(a)),"undefined"==typeof b)return("undefined"!=typeof a.required&&a.required===!0||"undefined"==typeof a.required&&this.jsoneditor.options.required_by_default===!0)&&k.push({path:c,property:"required",message:this.translate("error_notset")}),k;
// `enum`
if(a["enum"]){for(d=!1,g=0;g<a["enum"].length;g++)l===JSON.stringify(a["enum"][g])&&(d=!0);d||k.push({path:c,property:"enum",message:this.translate("error_enum")})}
// `extends` (version 3)
if(a["extends"])for(g=0;g<a["extends"].length;g++)k=k.concat(this._validateSchema(a["extends"][g],b,c));
// `allOf`
if(a.allOf)for(g=0;g<a.allOf.length;g++)k=k.concat(this._validateSchema(a.allOf[g],b,c));
// `anyOf`
if(a.anyOf){for(d=!1,g=0;g<a.anyOf.length;g++)if(!this._validateSchema(a.anyOf[g],b,c).length){d=!0;break}d||k.push({path:c,property:"anyOf",message:this.translate("error_anyOf")})}
// `oneOf`
if(a.oneOf){d=0;var m=[];for(g=0;g<a.oneOf.length;g++){
// Set the error paths to be path.oneOf[i].rest.of.path
var n=this._validateSchema(a.oneOf[g],b,c);for(n.length||d++,i=0;i<n.length;i++)n[i].path=c+".oneOf["+g+"]"+n[i].path.substr(c.length);m=m.concat(n)}1!==d&&(k.push({path:c,property:"oneOf",message:this.translate("error_oneOf",[d])}),k=k.concat(m))}
// `type` (both Version 3 and Version 4 support)
if(
// `not`
a.not&&(this._validateSchema(a.not,b,c).length||k.push({path:c,property:"not",message:this.translate("error_not")})),a.type)
// Union type
if(Array.isArray(a.type)){for(d=!1,g=0;g<a.type.length;g++)if(this._checkType(a.type[g],b)){d=!0;break}d||k.push({path:c,property:"type",message:this.translate("error_type_union")})}else this._checkType(a.type,b)||k.push({path:c,property:"type",message:this.translate("error_type",[a.type])});
// `disallow` (version 3)
if(a.disallow)
// Union type
if(Array.isArray(a.disallow)){for(d=!0,g=0;g<a.disallow.length;g++)if(this._checkType(a.disallow[g],b)){d=!1;break}d||k.push({path:c,property:"disallow",message:this.translate("error_disallow_union")})}else this._checkType(a.disallow,b)&&k.push({path:c,property:"disallow",message:this.translate("error_disallow",[a.disallow])});/*
...
d=b/o===Math.floor(b/o),
// Use math.js is available
window.math?d=window.math.mod(window.math.bignumber(b),window.math.bignumber(o)).equals(0):window.Decimal&&(d=new window.Decimal(b).mod(new window.Decimal(o)).equals(0)),d||k.push({path:c,property:a.multipleOf?"multipleOf":"divisibleBy",message:this.translate("error_multipleOf",[o])})}
// `maximum`
a.hasOwnProperty("maximum")&&(
// Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
d=a.exclusiveMaximum?b<a.maximum:b<=a.maximum,
// Use math.js is available
window.math?d=window.math[a.exclusiveMaximum?"smaller":"smallerEq"](window.math.bignumber(b),window.math.bignumber(a.maximum)):window.Decimal&&(d=new window.Decimal(b)[a.exclusiveMaximum?"lt":"lte"](new window.Decimal(a.maximum))),d||k.push({path:c,property:"maximum",message:this.translate(a.exclusiveMaximum?"error_maximum_excl":"error_maximum_incl",[a.maximum])})),
// `minimum`
a.hasOwnProperty("minimum")&&(
// Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
d=a.exclusiveMinimum?b>a.minimum:b>=a.minimum,
// Use math.js is available
window.math?d=window.math[a.exclusiveMinimum?"larger":"largerEq"](window.math.bignumber(b),window.math.bignumber(a.minimum)):window.Decimal&&(d=new window.Decimal(b)[a.exclusiveMinimum?"gt":"gte"](new window.Decimal(a.minimum))),d||k.push({path:c,property:"minimum",message:this.translate(a.exclusiveMinimum?"error_minimum_excl":"error_minimum_incl",[a.minimum])}))}else if("string"==typeof b)
// `maxLength`
a.maxLength&&(b+"").length>a.maxLength&&k.push({path:c,property:"maxLength",message:this.translate("error_maxLength",[a.maxLength])}),
// `minLength`
a.minLength&&(b+"").length<a.minLength&&k.push({path:c,property:"minLength",message:this.translate(1===a.minLength?"error_notempty":"error_minLength",[a.minLength])}),
// `pattern`
a.pattern&&(new RegExp(a.pattern).test(b)||k.push({path:c,property:"pattern",message:this.translate("error_pattern",[a.pattern])}));else if("object"==typeof b&&null!==b&&Array.isArray(b)){
// `items` and `additionalItems`
if(a.items)
// `items` is an array
if(Array.isArray(a.items))for(g=0;g<b.length;g++)
// If this item has a specific schema tied to it
// Validate against it
if(a.items[g])k=k.concat(this._validateSchema(a.items[g],b[g],c+"."+g));else{if(a.additionalItems===!0)break;if(!a.additionalItems){if(a.additionalItems===!1){k.push({path:c,property:"additionalItems",message:this.translate("error_additionalItems")});break}break}k=k.concat(this._validateSchema(a.additionalItems,b[g],c+"."+g))}else
// Each item in the array must validate against the schema
for(g=0;g<b.length;g++)k=k.concat(this._validateSchema(a.items,b[g],c+"."+g));
// `uniqueItems`
if(
// `maxItems`
a.maxItems&&b.length>a.maxItems&&k.push({path:c,property:"maxItems",message:this.translate("error_maxItems",[a.maxItems])}),
// `minItems`
a.minItems&&b.length<a.minItems&&k.push({path:c,property:"minItems",message:this.translate("error_minItems",[a.minItems])}),a.uniqueItems){var p={};for(g=0;g<b.length;g++){if(d=JSON.stringify(b[g]),p[d]){k.push({path:c,property:"uniqueItems",message:this.translate("error_uniqueItems")});break}p[d]=!0}}}else if("object"==typeof b&&null!==b){
// `maxProperties`
if(a.maxProperties){d=0;for(g in b)b.hasOwnProperty(g)&&d++;d>a.maxProperties&&k.push({path:c,property:"maxProperties",message:this.translate("error_maxProperties",[a.maxProperties])})}
// `minProperties`
if(a.minProperties){d=0;for(g in b)b.hasOwnProperty(g)&&d++;d<a.minProperties&&k.push({path:c,property:"minProperties",message:this.translate("error_minProperties",[a.minProperties])})}
// Version 4 `required`
if("undefined"!=typeof a.required&&Array.isArray(a.required))for(g=0;g<a.required.length;g++)"undefined"==typeof b[a.required[g]]&&k.push({path:c,property:"required",message:this.translate("error_required",[a.required[g]])});
// `properties`
var q={};for(g in a.properties)a.properties.hasOwnProperty(g)&&(q[g]=!0,k=k.concat(this._validateSchema(a.properties[g],b[g],c+"."+g)));
// `patternProperties`
if(a.patternProperties)for(g in a.patternProperties)if(a.patternProperties.hasOwnProperty(g)){var r=new RegExp(g);
// Check which properties match
for(i in b)b.hasOwnProperty(i)&&r.test(i)&&(q[i]=!0,k=k.concat(this._validateSchema(a.patternProperties[g],b[i],c+"."+i)))}
// `additionalProperties`
if(
// The no_additional_properties option currently doesn't work with extended schemas that use oneOf or anyOf
"undefined"!=typeof a.additionalProperties||!this.jsoneditor.options.no_additional_properties||a.oneOf||a.anyOf||(a.additionalProperties=!1),"undefined"!=typeof a.additionalProperties)for(g in b)if(b.hasOwnProperty(g)&&!q[g]){
// No extra properties allowed
if(!a.additionalProperties){k.push({path:c,property:"additionalProperties",message:this.translate("error_additional_properties",[g])});break}if(a.additionalProperties===!0)break;k=k.concat(this._validateSchema(a.additionalProperties,b[g],c+"."+g))}
// `dependencies`
if(a.dependencies)for(g in a.dependencies)if(a.dependencies.hasOwnProperty(g)&&"undefined"!=typeof b[g])
// Property dependency
if(Array.isArray(a.dependencies[g]))for(i=0;i<a.dependencies[g].length;i++)"undefined"==typeof b[a.dependencies[g][i]]&&k.push({path:c,property:"dependencies",message:this.translate("error_dependency",[a.dependencies[g][i]])});else k=k.concat(this._validateSchema(a.dependencies[g],b,c))}
// Custom type validation (global)
// Custom type validation (instance specific)
return f(h.defaults.custom_validators,function(d,e){k=k.concat(e.call(j,a,b,c))}),this.options.custom_validators&&f(this.options.custom_validators,function(d,e){k=k.concat(e.call(j,a,b,c))}),k},_checkType:function(a,b){
// Simple types
// Simple types
...
button_collapse:"Collapse",/**
    ...
    */
button_expand:"Expand"},h.plugins={ace:{theme:""},SimpleMDE:{},sceditor:{},select2:{},selectize:{}},f(h.defaults.editors,function(a,b){h.defaults.editors[a].options=b.options||{}}),h.defaults.resolvers.unshift(function(a){if("string"!=typeof a.type)return"multiple"}),h.defaults.resolvers.unshift(function(a){
// If the schema is a simple type
if(!a.type&&a.properties)return"object"}),h.defaults.resolvers.unshift(function(a){
// If the schema is a simple type
if("string"==typeof a.type)return a.type}),h.defaults.resolvers.unshift(function(a){if("integer"===a.type&&"rating"===a.format)return"rating"}),h.defaults.resolvers.unshift(function(a){if("boolean"===a.type)
...
if(a.oneOf||a.anyOf)return"multiple"}),h.defaults.resolvers.unshift(function(a){if("string"===a.type&&"starrating"===a.format)return"starrating"}),function(){if(window.jQuery||window.Zepto){var a=window.jQuery||window.Zepto;a.jsoneditor=h.defaults,a.fn.jsoneditor=function(a){var b=this,c=this.data("jsoneditor");if("value"===a){if(!c)throw"Must initialize jsoneditor before getting/setting the value";
// Set value
